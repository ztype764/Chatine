...
public class ClientHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            while (true) {
                ...
                if (ChatServer.isClientBanned(clientName)) {
                    out.println("BANNED");
                    socket.close();
                    return;
                }
            ...
            logger.log(Level.SEVERE, "Error in client handler");
        } finally {
            try {
                socket.close();
            } catch (IOException e) {
            ...
            if (ChatServer.isClientBanned(clientName)) {
                logger.info(clientName + " has been banned.");
            } else {
    ...
    public void sendBanMessage() {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Error closing socket after ban", e);
        }
    }
}
